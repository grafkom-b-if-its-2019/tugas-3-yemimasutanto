...
let YVertexPositionBuffer
let YVertexColorBuffer
let YVertexIndexBuffer
...
function initBuffers() {
    ...
    let vertices = [
        ...
        // vertical inner right (cee'c')
        2.00, 3.00, 0.00,
        0.00, 1.00, 0.00,
        0.00, 1.00, -1.00,
        2.00, 3.00, -1.00,

        // vertical outer right (dgg'd')
        3.00, 2.00, 0.00,
        1.00, 0.00, 0.00,
        1.00, 0.00, -1.00,
        3.00, 2.00, -1.00,
        ...
        // vertical inner left (bee'b')
        -2.00, 3.00, 0.00,
        0.00, 1.00, 0.00,
        0.00, 1.00, -1.00,
        -2.00, 3.00, -1.00,

        // vertical outer left (aff'a')
        -3.00, 2.00, 0.00,
        -1.00, 0.00, 0.00,
        -1.00, 0.00, -1.00,
        -3.00, 2.00, -1.00,
    ...
    YVertexPositionBuffer.numItems = 64

    // Triangle Color
    YVertexColorBuffer = gl.createBuffer()
    gl.bindBuffer(gl.ARRAY_BUFFER, YVertexColorBuffer)
    let colors = []
    
    // front and back
    for (let i = 0; i < 24; i++) {
        colors = colors.concat([0.0, 1.0, 0.0, 1.0]) //hijau
    }

    // inner and outer vertical
    for (let i = 0; i < 24; i++) {
        colors = colors.concat([0.0, 0.0, 1.0, 0.0])//putih
    }
    ...
    YVertexColorBuffer.itemSize = 4
    YVertexColorBuffer.numItems = 64
    
    YVertexIndexBuffer = gl.createBuffer()
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, YVertexIndexBuffer)
    let YVertexIndices = []
    for (let i = 0; i < 16; i++) {
        YVertexIndices = YVertexIndices.concat([i*4, i*4 + 1, i*4 + 2])
        YVertexIndices = YVertexIndices.concat([i*4, i*4 + 2, i*4 + 3])
    }
    ...
    for (let i = 0; i < vertices.length / 3; i++) {
        colors = colors.concat([0.0, 0.0, 1.0, 0.0])        
    }
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(colors), gl.STATIC_DRAW)
...
function drawScene() {
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, YVertexColorBuffer)
    gl.vertexAttribPointer(shaderProgram.vertexColorAttribute, YVertexColorBuffer.itemSize, gl.FLOAT, false, 0, 0)
